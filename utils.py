...
def detect_base_language(localizations: List[Dict]) -> Optional[str]:
    ...
    available_locales = [
        loc.get("attributes", {}).get("locale") for loc in localizations
    ]
...
def export_existing_localizations(
    localizations_data: List[Dict[str, Any]],
    ...
    clean_app_name = "".join(c.lower() for c in app_name if c.isalnum() or c in "-_")[
        :20
    ]
